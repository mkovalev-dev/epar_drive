...
import { headerFolderData } from "../pages/slice/sliceFolder";

export default function HeadActionCreate() {
  const [visibleModalCreate, setVisibleModalCreate] = useState(false);
  const dispatch = useDispatch();
  const { id } = useParams();
  const stateFolderRetrieveListData = useSelector(headerFolderData);
  return (
    <>
      {!stateFolderRetrieveListData?.read_only && (
        <>
          <Button
            icon={<PlusOutlined />}
            block
